...
public class ClickHolder {
    ...
    public void handleRightClick(InventoryClickEvent event, Player player, ItemStack cursor, int slot, PlacedChestContent vault, PlacedChest chest, int page, int inventorySize) {
        ...
        if (chest.getChestTemplate().isInfinite()) {
            if(cursor == null || cursor.getType().isAir() && !vaultItem.isEmpty()) {
                int amountToRemove = Math.min(vaultItem.amount() / 2, plugin.getManager(StoragePlusManager.class).getMaxStackSize(chest, vaultItem.item()) / 2);
                if(amountToRemove == 0) {
                    amountToRemove = 1;
                }
                //Remove item
                ...
                int slotToAdd = this.findCorrespondingSlot(cursor, vault, chest);
                if(slotToAdd == -1) {
                    return;
                }
    ...
    public void handleShift(InventoryClickEvent event, Player player, ItemStack cursor, ItemStack current, int slot, int inventorySize, PlacedChestContent vault, PlacedChest chest, List<Integer> slots, int page) {
        ...
        if(chest.getChestTemplate().isInfinite()) {
            if (slotInInventory >= inventorySize) {
                ...
                int slotToAdd = this.findCorrespondingSlot(current, vault, chest);
                if(slotToAdd == -1) {
                    return;
                }
    ...
    public void handleDrop(InventoryClickEvent event, Player player, int slot, PlacedChestContent vault, PlacedChest chest, boolean controlDrop, int page, int inventorySize) {
        ...
        int amountToDrop;
        if(controlDrop) {
            amountToDrop = Math.min(vaultItem.amount(), plugin.getManager(StoragePlusManager.class).getMaxStackSize(chest, vaultItem.item()));
        } else {
            amountToDrop = 1;
        }
    ...
    private int addItemInChest(PlacedChestContent content, ItemStack item, int amount, PlacedChest chest, int page, Inventory chestInventory, Player player) {
        ...
        while (leftAmount > 0) {
            ...
            int amountToAdd = Math.min(amount, maxStackSize - storageItem.amount());
